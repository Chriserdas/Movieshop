...
DELIMITER$
CREATE DEFINER = CURRENT_USER TRIGGER `after_insert_film_log` AFTER INSERT ON `film_rental` 
FOR EACH ROW
BEGIN
    declare date_time datetime;
	set date_time=now();
    insert into log values
    (new.customer_id, 'film_rental', 'insert', date_time, 'yes');
END$
DELIMITER;
...
/* delete from log where username= 16;
delete from film_rental where film_rental_id=17000; */

DROP TRIGGER IF EXISTS after_update_film_log;
DELIMITER$
CREATE DEFINER = CURRENT_USER TRIGGER `after_update_film_log` 
AFTER UPDATE ON `film_rental` FOR EACH ROW
BEGIN
    declare date_time datetime;
	set date_time=now();
    insert into log values
    (new.customer_id, 'film_rental', 'update', date_time, 'yes');
END$
DELIMITER;
...
/* delete from log where username=512;
UPDATE film_rental
SET film_rental_id= 2029
where film_rental_id = 2028; */

DROP TRIGGER IF EXISTS after_delete_film_log;
DELIMITER$
CREATE DEFINER = CURRENT_USER TRIGGER `after_delete_film_log` 
AFTER DELETE ON `film_rental` FOR EACH ROW
BEGIN
    declare date_time datetime;
	set date_time=now();
    insert into log values
    (old.customer_id, 'film_rental', 'delete', date_time, 'yes');
END$
DELIMITER;

/*DELETE FROM film_rental where film_rental_id=1313;
select * from log; */

/* delete from log where username=448;
insert into film_rental VALUES
(1313, '2005-06-15 10:18:34', 361,448); */
...
DELIMITER$
CREATE DEFINER=`root`@`localhost` TRIGGER `after_insert_fpaym_log` 
AFTER INSERT ON `film_payment` 
FOR EACH ROW BEGIN
    declare date_time datetime;
	set date_time=now();
    insert into log values
    (new.customer_id, 'film_payment', 'insert', date_time, 'yes');
END$
DELIMITER;

DROP TRIGGER IF EXISTS after_update_fpaym_log;
DELIMITER$
CREATE DEFINER=`root`@`localhost` TRIGGER `after_update_fpaym_log` 
...
FOR EACH ROW BEGIN
	...
	set date_time=now();
    insert into log values
    (new.customer_id, 'film_payment', 'update', date_time, 'yes');
END$
DELIMITER;

DROP TRIGGER IF EXISTS after_delete_fpaym_log;
DELIMITER$
CREATE DEFINER=`root`@`localhost` TRIGGER `after_delete_fpaym_log` 
...
FOR EACH ROW BEGIN
	...
	set date_time=now();
    insert into log values
    (old.customer_id, 'film_payment', 'delete', date_time, 'yes');
END$
DELIMITER;
...
DELIMITER$
CREATE DEFINER = CURRENT_USER TRIGGER `after_insert_ep_log` 
AFTER INSERT ON `episode_rental` FOR EACH ROW
BEGIN
declare date_time datetime;
	set date_time=now();
    insert into log values
    (new.customer_id, 'episode_rental', 'insert', date_time, 'yes');
END$
DELIMITER;
...
DELIMITER$
CREATE DEFINER = CURRENT_USER TRIGGER `after_update_ep_log` 
...
declare date_time datetime;
	...
	set date_time=now();
    insert into log values
    (new.customer_id, 'episode_rental', 'update', date_time, 'yes');
END$
DELIMITER;

DROP TRIGGER IF EXISTS after_delete_ep_log;
DELIMITER$
CREATE DEFINER = CURRENT_USER TRIGGER `after_delete_ep_log` 
AFTER DELETE ON `episode_rental` FOR EACH ROW
BEGIN
declare date_time datetime;
	set date_time=now();
    insert into log values
    (old.customer_id, 'episode_rental', 'delete', date_time, 'yes');
END$
DELIMITER;
...
DELIMITER$
CREATE DEFINER = CURRENT_USER TRIGGER `after_insert_eppaym_log` 
...
declare date_time datetime;
	...
	set date_time=now();
    insert into log values
    (new.customer_id, 'episode_payment', 'insert', date_time, 'yes');
END$
DELIMITER;
...
DELIMITER$
CREATE DEFINER = CURRENT_USER TRIGGER `after_update_eppaym_log` 
...
declare date_time datetime;
	...
	set date_time=now();
    insert into log values
    (new.customer_id, 'episode_payment', 'update', date_time, 'yes');
END$
DELIMITER;

DROP TRIGGER IF EXISTS after_update_eppaym_log;
DELIMITER$
CREATE DEFINER = CURRENT_USER TRIGGER `after_delete_eppaym_log` 
...
declare date_time datetime;
	...
	set date_time=now();
    insert into log values
    (old.customer_id, 'episode_payment', 'delete', date_time, 'yes');
END$
DELIMITER;
